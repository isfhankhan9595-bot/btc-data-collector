...
def test_generate_label_columns_drops_nan_labels_for_one_second_horizon():
    ...
    labeled_df, _, _ = _generate_label_columns(df, grid_ms=100, threshold=0.00005, horizons_s=[1])
...
def test_generate_label_columns_drops_exactly_longest_horizon_shift_rows(capsys):
    ...
    labeled_df, rows_dropped, _ = _generate_label_columns(df, grid_ms=100, threshold=0.00005, horizons_s=[1, 2])
